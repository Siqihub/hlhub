...
登录成功后，AutoDy 会在已确认的聊天页执行一次只读好友扫描；管理台启动和登录健康检查会先显示 `data/discovered_friends.json` 中的本地缓存，并仅在缓存超过 24 小时时后台刷新一次。扫描通过 DOM 读取并有限滚动聊天列表，候选记录包含稳定本地 ID、首次/最近发现时间、当前或历史状态和头像关联。扫描失败不会清空上一次有效候选，未在本次扫描中出现的条目会保留为历史候选。

头像只保存在 `data/avatar-cache/`：首次发现、头像缺失或头像缓存超过 7 天时才截取可见头像元素的本地缩略图；未过期缓存会直接复用。管理台通过本地接口显示头像，既不会保存抖音头像 URL，也不会上传或打包头像。后台扫描与发送任务共用浏览器锁；发送进行中时扫描会延后，扫描全程不会输入或发送消息。
...
新备份使用带版本、类别和 SHA-256 校验和的 manifest。导入前会预检路径、可执行文件、冲突和设置变化；支持合并/替换，并会先在 `data/backups/` 创建本地回滚备份。完整备份会保留好友配置（包括启用状态、备注和稳定本地 ID），但明确排除 `data/browser-profile`、Cookie、令牌、日志、截图、`data/avatar-cache/` 和 `data/discovered_friends.json`。换电脑后导入备份，仍需本人重新扫码登录。
...
- `data/discovered_friends.json`：原子写入的候选好友缓存、扫描结果与新鲜度状态；不进入 Git、诊断包、便携包或备份
