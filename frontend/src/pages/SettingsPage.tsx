  ...
  return (
    <section className="editor-page">
      ...
      <div className="panel settings-form">
        ...
        <label><span>每日发送时间<small>错过该时间后才允许启动同日恢复</small></span><input aria-label="每日发送时间" type="time" value={config.daily_send_time} onChange={(event) => setConfig({ ...config, daily_send_time: event.target.value })} /></label>
        <label><span>同日恢复截止时间<small>超过该时间不再自动补跑</small></span><input aria-label="恢复截止时间" type="time" value={config.recovery_deadline} onChange={(event) => setConfig({ ...config, recovery_deadline: event.target.value })} /></label>
